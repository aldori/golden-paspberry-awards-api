...
describe('Studio Repository', () => {
  beforeAll(async () => {
    await database.exec("CREATE TABLE studios (id_studio INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL)");
  });
  ...
  it('should update an existing studio', async () => {
    ...
    const updatedStudio = new Studio(studioToUpdate.id_studio, 'updated studio');

    await StudioRepository.update(studioToUpdate.id_studio, updatedStudio);
    const foundStudio = await StudioRepository.findById(studioToUpdate.id_studio);
